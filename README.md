# Media Archive Organizer (v2.6.4)
...
**Language**: Python (v2.6.4 Update)

## New Features (v2.6.4)
- **Visual Feedback**:
  - **Live Filename**: The AI Scanner now shows exactly which file is being scanned right under the progress bar.
- **Smart Logging**:
  - **Focus on Action**: The console no longer floods you with "Scanning..." messages. It now only highlights files marked for moving (e.g., `[MOVE] >> IMG_1234.jpg`).

## New Features (v2.6.3)
- **UI Polish**:
  - **Compact Console Grip**: Removed bulky arrows and reduced padding. The splitter is now a sleek, minimal line.
  - **Clean Controls**: Renamed "Show Log Output" to "Log Output" for consistency.
- **AI Scanner**:
  - **Log Control**: Added a "Log Output" checkbox to the AI Scanner, giving you control over the new real-time logging features.

## New Features (v2.6.2)
- **UI Refinement**:
  - **Console Grip**: Added a visual grip bar with dynamic arrows (▲/▼) to clearly indicate the resizeable console area.
- **Workflow**:
  - **Seamless Moving**: Moving files no longer interrupts you with a popup. Instead, every move is logged in real-time to the console.

## New Features (v2.6.1)
- **Hotfix**:
  - **Crash Fix**: Resolved a startup crash caused by an invalid UI component (`CTkPanedWindow`). Replaced with standard `PanedWindow` to ensure stability.
...
## New Features (v2.5.9)
- **Visual Polish (Final)**:
  - **Layout Lock**: Fixed a bug where the preview panel would still expand to fit large images. The layout is now truly locked (`pack_propagate(False)`).

## New Features (v2.5.8)
- **Visual Polish**:
  - **Aspect Ratio Fix**: Photos now scale correctly without being stretched or distorted, regardless of window size.
  - **Layout Lock**: The preview panel is now rigidly sized, so loading images never causes the file lists to jump or shift.

## New Features (v2.5.7)
- **UI Stability**:
  - **Jitter Fix**: Fixed an issue where the layout would dance/resize aggressively when clicking different shaped photos.
  - **Smart Resizing**: The preview window now intelligently waits for you to stop resizing before updating the image (Debouncing), making the window much smoother to drag.

## New Features (v2.5.6)
- **Performance Upgrade**:
  - Implemented **Multithreaded Image Loading** for the preview window. The UI no longer freezes when clicking large photos; images load in the background and pop in when ready.

## New Features (v2.5.5)
- **Hotfix 2**:
  - Validated fix for `NameError` crash when using the "Move Files" button (previous hotfix failed to apply correctly).
